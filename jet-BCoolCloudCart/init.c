...
void UserDefBoundary (const Data *d, RBox *box, int side, Grid *grid)
{
  int   i, j, k, nv;
  double  *x1, *x2, *x3;
  double B0,B,P,Rj,Pj,r0,rstar,beta,r;
  ...
  if (side == 0){
    TOT_LOOP(k,j,i){
      if ((fabs(x1[i])<=2.0*g_inputParam[jet_window]) & ((x2[j] >= 0)&(x2[j] < 0.05)) ) {
        ...
        if (g_inputParam[gBphi]==1) { // if we have a magnetic field in jet
          ...
          d->Vc[BX1][k][j][i] = -B*x3[k]/r;
          d->Vc[BX3][k][j][i] = B*x1[i]/r;
  ...
  if (side == X1_END){  /* -- X1_END boundary -- */
    ...
    if (side == X2_BEG){  /* -- X2_BEG boundary -- */
        if (box->vpos == CENTER) {
            BOX_LOOP(box,k,j,i){
            if (g_inputParam[gJET]==1) { // if we have a jet
