...
PU=sim['PU'].item();print('Loaded simulation units')
...
vars = {'RHO': np.log10(sim['RHO']),
        'TMP': np.log10(sim['TMP']),
        'Gamma':gamma,
        'Sigma':sigma
        #'Vz':sim['Vz']
}
...
varlvl={'RHO': np.linspace(sim['RHO'].min(),sim['RHO'].max(),50),
      'TMP': np.linspace(sim['TMP'].min(),sim['TMP'].max(),50),
